...
function CustomPieChart() {
  const rs = '\u20B9';
  const data = [
    {key: 'red', value: 160, color: '#85E0A3', label: 'Sales'},
    {key: 'green', value: 240, color: '#FFD966', label: 'Purchase'},
    {key: 'blue', value: 150, color: '#80CAFF', label: 'Expenses'},
  ];
          ...
          return (
            <G key={item.key}>
              ...
              <SvgText fill="#000" fontSize="16" textAnchor="middle">
                <SvgText x={labelX} y={labelY - 10} style={{fontSize: 15, fontWeight: '400', color: '#707070'}}>
                  {item.label}
                </SvgText>
                <SvgText
                  x={labelX}
                  y={labelY + 10}
                  style={{fontSize: 15, fontWeight: '600', color: '#707070'}}>
                  {rs}
                  {item.value}
                </SvgText>
              </SvgText>
